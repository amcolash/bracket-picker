...
window.onload = () => {
  ...
  pager1.addEventListener('click', (e) => {
    e.stopPropagation();
    preview(0);
  });
  pager2.addEventListener('click', (e) => {
    e.stopPropagation();
    preview(1);
  });
  pager3.addEventListener('click', (e) => {
    e.stopPropagation();
    preview(2);
  });
  ...
  image1.addEventListener('load', () => {
    file1.innerHTML = getInfo(currentSet[0], image1, histogram1);
    section1.classList.remove('hidden');
    feather.replace();
  });
  image2.addEventListener('load', () => {
    file2.innerHTML = getInfo(currentSet[1], image2, histogram2);
    section2.classList.remove('hidden');
    feather.replace();
  });
  image3.addEventListener('load', () => {
    file3.innerHTML = getInfo(currentSet[2], image3, histogram3);
...
function checkKey(e) {
  ...
  if (e.keyCode == '13' && e.ctrlKey) {
    ...
    // esc
    hideOverlay();
  } else if ((e.keyCode == '32' || e.keyCode == '82') && !e.ctrlKey) {
    // space, r
    if (overlayShown()) {
      select(previewIndex);
    } else {
